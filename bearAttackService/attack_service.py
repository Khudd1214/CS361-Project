...
# get working location, for non-python script users
...
def read_request():
    ...
    input_file = open(os.path.join(__location__, 'attack_request.csv'))
...
def write_invalid():
    ...
    with open(os.path.join(__location__, 'attack_response.csv'), 'w') as outfile:
...
def delete_request_file():
    ...
    :return:
    """
    if os.path.exists(__location__ + "/attack_request.csv"):
        os.remove(__location__ + "/attack_request.csv")
